...
def plot_calibration_curve(gaus_pred, errors_observed, mis_cal):
    ...
    ax.fill_between(gaus_pred, gaus_pred, errors_observed, color="#807dba", alpha=0.8, label="Miscalibration Area = {:0.3f}".format(mis_cal))
    ...
    ax.set_xlabel("Expected Fraction of Errors", fontsize=20)
    ax.set_ylabel("Observed Fraction of Errors", fontsize=20)
...
def get_slope_metric(uq_ordered, errors_ordered, Nbins=10, include_bootstrap=True):
    ...
    ax.errorbar(x, y, yerr = assymetric_errors, fmt="o", linewidth=2, color='#807dba')
    ax.plot(np.arange(rmvs[0],rmvs[-1],0.0001),np.arange(rmvs[0],rmvs[-1],0.0001), linestyle='dashed', color='k')
    ax.plot(rmvs, y_pred, linestyle="dashed", color='red', label=r'$R^2$ = '+"{:0.2f}".format(r_sq)+", \nSlope = {:0.2f}".format(slope)+", \nIntercept = {:0.2f}".format(intercept))

    ax.set_xlabel("RMV", fontsize=28)
